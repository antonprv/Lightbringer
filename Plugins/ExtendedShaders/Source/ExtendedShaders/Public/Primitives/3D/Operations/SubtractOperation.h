...
#include "SubtractOperation.generated.h"

/**
 * Signed Distant Field Sphere, rendered using raymarching
 */
UCLASS(
    meta = (DisplayName = "Subtract Operation",
        Category = "3D Primitives Operations",
        ToolTip = "Performs a boolean subtraction on any two 3D Parts given."))
class EXTENDEDSHADERS_API USubtractOperation : public UCustomHLSLExpression
{
    GENERATED_BODY()

public:
    USubtractOperation(const FObjectInitializer& ObjInit);
